...
export default class NoRecurrentEvent extends SetEvent {
  ...
  setEvent (event) {
    ...
    let hourStart = event.getHourStart(), hourEnd = event.getHourEnd(), td, tds, start, end, i, column;
    ...
    if (event.isOneDay()) {
      ...
      if (this.calendar.eventInColumn[parseInt(column) -1] >= 6) {
        ...
        for (i = hourStart; i <= hourEnd; i++) {
          ...
          this.createEventElement(tds[i].querySelector('.eventDays'), parseInt(column), i === hourStart);
